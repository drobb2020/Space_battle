...
def yellow_handle_movement(keys_pressed, yellow):
  ...
  if keys_pressed[pygame.K_w] and yellow.y - VEL > 0: #Move up
    yellow.y -= VEL
  if keys_pressed[pygame.K_x] and yellow.y + VEL + yellow.height < HEIGHT - 15: #Move down
    yellow.y += VEL
...
def handle_bullets(yellow_bullets, red_bullets, yellow, red):
  for bullet in yellow_bullets:
    ...
    elif bullet.x > WIDTH: # Bullet hit the edge of the screen
      yellow_bullets.remove(bullet)
  ...
  for bullet in red_bullets:
    ...
    elif bullet.x < 0: # Bullet hit the edge of the screen
      red_bullets.remove(bullet)
...
def main():
  ...
  while run:
    ...
    for event in pygame.event.get():
      ...
      if event.type == pygame.KEYDOWN: #Fire bullets for yellow
        if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
          bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)
          yellow_bullets.append(bullet)
          BULLET_FIRE_SOUND.play()

        if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS: #Fire bullet for red
